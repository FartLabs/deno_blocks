...
import { getDeploymentAppLogs } from "#/lib/subhosting_api/mod.ts";
...
export const handler: Handlers = {
  async GET(request: Request, context: FreshContext) {
    ...
    const deploymentID = context.params.deployment_id;
    if (!deploymentID) {
      return new Response("Expected deployment_id", { status: 400 });
    }
    ...
    const deployments = await getDeploymentAppLogs({
